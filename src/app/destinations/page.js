...
//                 you plan the perfect trip tailored to your preferences.
//               </p>
//               <div className="flex flex-col sm:flex-row gap-5 justify-center">
//                 <a
//                   href="tel:8460146012"
...
export default function DestinationsPage() {
  ...
  const filteredDestinations = useMemo(() => {
    ...
    if (durationFilter !== "all") {
      filtered = filtered.filter((dest) => {
        const dur = dest.duration?.days
          ? `${dest.duration.days} Days ${dest.duration.nights} Nights`
          : dest.duration;
        return dur === durationFilter;
      });
    }
  ...
  return (
    <div className="w-full overflow-hidden bg-gray-50">
      ...
      {/* Search & Filters */}
      <section className="sticky top-20 bg-white/95 backdrop-blur-md z-50 border-b-2 border-gray-200 shadow-md py-6">
        <div className="container mx-auto px-4 flex flex-col lg:flex-row gap-4 items-center justify-between">
          <input
            ...
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="flex-1 px-4 py-3 border rounded-2xl focus:ring-2 focus:ring-secondary border-gray-300 shadow-sm"
          />

          <select
            value={durationFilter}
            onChange={(e) => setDurationFilter(e.target.value)}
            className="px-4 py-3 border rounded-2xl focus:ring-2 focus:ring-secondary border-gray-300 shadow-sm"
          >
            <option value="all">All Durations</option>
            {Array.from(
              new Set(
                destinations
                  .map((d) => (d.duration?.days ? `${d.duration.days} Days ${d.duration.nights} Nights` : d.duration))
                  .filter(Boolean)
              )
            ).map((duration) => (
              <option key={duration} value={duration}>
      ...
      {/* Destinations Grid */}
      <main className="container mx-auto px-4 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {filteredDestinations.length === 0 ? (
          <p className="text-center text-gray-600 col-span-full">No destinations found.</p>
        ) : (
