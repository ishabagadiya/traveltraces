    ...
    <section className="relative z-20 bg-[#dfdfdf] px-4 pb-8 pt-2 md:px-8 md:pb-12">
      <div className="mx-auto w-full max-w-4xl min-w-0 -mb-20 md:-mb-28">
        <div className="flex min-w-0 flex-col overflow-hidden rounded-[1.35rem] bg-white shadow-[0_12px_40px_rgba(0,0,0,0.12)] sm:min-h-[140px] sm:flex-row sm:rounded-[1.75rem]">
          ...
          <div className="relative flex w-full min-w-0 flex-1 flex-col items-center sm:items-start justify-center px-5 py-4 sm:py-8 sm:px-6 md:px-10">
            <p className="mb-3 w-full text-center sm:text-left text-pretty text-xs md:text-base font-medium leading-snug text-gray-900 sm:mb-2">
            ...
            <div className="flex w-full min-w-0 flex-nowrap items-center justify-center sm:justify-start gap-2.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] sm:gap-2 md:gap-2.5 [&::-webkit-scrollbar]:hidden">
              <Link
                ...
                aria-label="WhatsApp"
                className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white shadow-sm transition active:scale-95"
              >
                <FaWhatsapp className="h-[1.125rem] w-[1.125rem]" aria-hidden />
              </Link>
              <a
                href={`tel:${PHONE_TEL}`}
                aria-label={`Call ${PHONE_DISPLAY}`}
                className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-secondary text-white shadow-sm ring-1 ring-black/[0.06] transition active:scale-95"
              >
                <FaPhoneAlt className="h-3 w-3 md:h-4 md:w-4" aria-hidden />
              </a>
          ...
          {/* Instagram */}
          <div className="relative flex w-full min-w-0 flex-1 flex-col items-center sm:items-start justify-center px-5 py-4 sm:py-8 sm:px-6 md:px-10">
            <p className="relative z-10 mb-3 w-full max-w-none text-center sm:text-left text-pretty text-xs md:text-base font-semibold leading-snug text-gray-900 sm:mb-2 md:leading-snug">
            ...
            >
              <BiLogoInstagram className="h-7 w-7 shrink-0 md:h-8 md:w-8" aria-hidden />
